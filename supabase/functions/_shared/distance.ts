// Rough city-centroid distance utilities for ranking input.
// We do NOT have per-veteran coordinates (only city), so distance is computed
// from the event's lat/lng to the centroid of the veteran's city.
...
/**
 * Best-effort distance from a veteran's home city to an event location.
 * Returns null when either side is unknown — caller decides what to do.
 */
